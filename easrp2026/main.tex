...
\easrpauthor{Saksham Mishra}
\easrpaffiliation{Department of Computer Science, University of Birmingham Dubai, UAE}
...
That chain is a cascade \citep{dobson2007chaos,carreras2002critical}.

Most operational tools, and much of the recent learning literature, still \emph{react} after something has already gone wrong \citep{huang2020adaptive}.
...
Security-constrained and AC optimal power flow remain the default preventive tools in operations research on grids \citep{capitanescu2016opf}.
They are principled and slow to re-solve from scratch.
We use pandapower's OPF \citep{thurner2018pandapower} as that baseline, not as a straw man.

\paragraph{Learning, usually after the fault.}
Deep RL has been applied to emergency load shedding and voltage control \citep{huang2020adaptive,duan2020voltage} and to sequential grid operation in the L2RPN setting \citep{yoon2021l2rpn}.
...
Deep RL results are sensitive to seeds and evaluation protocol \citep{henderson2018deep}.
A paper that only reports an agent beating ``do nothing'' on 25 days is easy to over-read.
We therefore report two seeds, a paired accident design, and a rule that a first-year student can recode.
...
We train 100 short episodes and two seeds.
...
It is a \emph{fair, small, explainable} test that a reviewer can actually rerun.
